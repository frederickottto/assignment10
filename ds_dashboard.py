...
plot_salary_min, plot_salary_max = int(df['Salary'].min()), int(df['Salary'].max())
plot_salary_range = st.slider(
    "Select salary range for plot (£)",
    min_value=plot_salary_min,
    max_value=plot_salary_max,
    value=(plot_salary_min, plot_salary_max)
)

top_titles = df['Job Title'].value_counts().head(10).index

plot_df = df[df['Job Title'].isin(top_titles)]
plot_df = plot_df[(plot_df['Salary'] >= plot_salary_range[0]) & (plot_df['Salary'] <= plot_salary_range[1])]
